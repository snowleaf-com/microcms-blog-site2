...
複数セクション（ご飯／他趣味／制作物など）を増やすときのルール。ヘッダーに方針プレビューあり（ローカル確認用）。

- セクションは別ルートにする（トップ内タブは使わない）
- 想定リンク: 飯ログ / 趣味ログ / 制作物 / About（当面 2〜4、制作物は個人作品への入口）
- **PC**: ハンバーガーにしない。ロゴ＋テキストリンク横並び
- **モバイルのみ**: ハンバーガー／ドロワー
- アクティブ表示は accent の下線または文字色のみ（pill だらけにしない）
- 制作物は自サイト内ページでも、外部ポートフォリオへのリンクでも可（開き方は実装時に決める）

## 目次（記事詳細）

- **PC（2カラム）**: 見出しがなくても右レールに目次枠を出し、中身は「目次なし」
- **1カラム**: 見出しがなければ目次自体を出さない（従来どおり）
